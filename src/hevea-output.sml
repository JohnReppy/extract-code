 ...
 *
 * Output device for producing files suitable for inclusion in
 * a document being processed by the HeVeA TeX to HTML tool.
 *)

structure HeVeADevice : OUTPUT_DEVICE =
  struct
   ...
   *)
    ...
    fun output {outStrm, codeEnv, text} = let
	  ...
	  fun prText ((T.Keyword | T.Symbol), text) = (pr "\\kw{"; pr' text; pr "}")
	    | prText (T.SpecialId, text) = (pr "\\specialid{"; pr' text; pr "}")
	    | prText (T.TyVar, text) = (pr "\\tyvar{"; pr' text; pr "}")
	    | prText (T.Comment, text) = (pr "\\comment{"; pr' text; pr "}")
	    | prText (T.Literal, text) = (pr "\\literal{"; pr' text; pr "}")
...
structure HeVeAOutput = OutputFn (HeVeADevice)
